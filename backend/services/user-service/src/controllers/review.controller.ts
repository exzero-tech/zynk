...
export const createUserReview = async (req: Request, res: Response): Promise<void> => {
  try {
    // Get reviewer ID from JWT middleware
    ...
    const result = await createReview(reviewerId, reviewedUserId, rating, comment);
...
export const updateUserReview = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const result = await updateReview(reviewId, reviewerId, rating, comment);
